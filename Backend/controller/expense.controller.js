...
export const addExpense = async (req, resp) => {
    try {
        ...
        return resp.status(201).json({ message: "New Expense added successfully", expense, success: true });
    } catch (error) {
...
export const getExpenseByUserId = async (req, resp) => {
    try {
        ...
        if (!expense || expense.length === 0) {
            return resp.status(404).json({ message: "No expense records found for this user", success: false });
        }
        
        return resp.status(200).json({ message: "Expense records fetched successfully", expense, success: true });
    } catch (error) {
        return resp.status(500).json({ message: "Internal server error", success: false });
    }
}
...
export const updateExpense = async (req, resp) => {
    try {
        ...
        if (!expense) {
            return resp.status(404).json({ message: "Expense not found", success: false });
        }
        
        return resp.status(200).json({ message: "Expense updated successfully", success: true });
    } catch (error) {
        return resp.status(500).json({ message: "Internal server error", success: false });
    }
}
...
export const deleteExpense = async (req, resp) => {
    try {
        ...
        return resp.status(200).json({ message: "Expense deleted successfully", success: true });
    } catch (error) {
        return resp.status(500).json({ message: "Internal server error", success: false });
    }
}
...
export const calculateTotalExpense = async (req, resp) => {
    try {
        ...
        if (!userExpenses || userExpenses.length === 0) {
            return resp.status(404).json({ message: "No expense found", success: false });
        }
        
        const totalExpense = userExpenses.reduce((sum, expense) => sum + (expense.amount || 0), 0);
        
        return resp.status(200).json({ message: "Total Expense calculated successfully", totalExpense, success: true });
    } catch (error) {
        return resp.status(500).json({ message: "Internal server error", success: false });
    }
}